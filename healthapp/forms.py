...
class HealthCustomForm(forms.ModelForm):
    ...
    def clean(self):
        ...
        # 전부 없음일경우 오류 발생
        if list_1[0] == '없음' and list_1[1] == '없음' and list_1[2] == '없음':
            self.add_error('exercise_1', "적어도 하나의 이상 운동을 선택해 주십시오")

        # 없음을 제외한 다른 운동의 경우 중복될 경우 오류 발생
        if len(set(list_1)) != 3 and list_1.count('없음') != 2 and list_1.count('없음') != 3:
            self.add_error('exercise_1', "중복은 허용되지 않습니다")

        # 없음이 다른 운동 앞에 올 경우 문제 발생
        if list_1.index('없음') == 0 and (list_1[list_1.index('없음') + 1] != '없음' or list_1[list_1.index('없음') + 2] != '없음'):
            self.add_error('exercise_1', "없음 다음에 운동이 올 수 없습니다. ")

        if list_1.index('없음') == 1 and (list_1[list_1.index('없음') + 1] != '없음'):
            self.add_error('exercise_1', "없음 다음에 운동이 올 수 없습니다. ")
